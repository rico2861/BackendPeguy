...
router.get('/predictions', authenticateOptional, async (req, res) => {
  ...
  const predictions = Prediction.listPredictions({ date, league, country, market, q }).map((p) =>
...
router.get('/predictions/:id', authenticateOptional, async (req, res) => {
  await trySync();
  const pred = Prediction.getPrediction(req.params.id);
...
router.get('/leagues', (req, res) => {
  res.json({ leagues: Prediction.listLeagues() });
});

router.get('/daily-bets', authenticateOptional, async (req, res) => {
  await trySync();
  const date = req.query.date || new Date().toISOString().slice(0, 10);
  res.json({ date, tickets: Prediction.dailyTickets(date, req.user) });
});
...
router.post('/predictions', authenticate, authorize('moderator', 'admin'), (req, res) => {
  const data = req.body || {};
  const missing = REQUIRED_FIELDS.filter((f) => !data[f]);
  if (missing.length) {
    return res.status(400).json({ error: `Champs manquants : ${missing.join(', ')}` });
  }
  const pred = Prediction.createPrediction(data, req.user.id, data.created_by_name || req.user.email);
...
router.put('/predictions/:id', authenticate, authorize('moderator', 'admin'), (req, res) => {
  const existing = Prediction.getPrediction(req.params.id);
  ...
  const updated = Prediction.updatePrediction(req.params.id, req.body || {});
  res.json({ prediction: updated });
});

router.delete('/predictions/:id', authenticate, authorize('moderator', 'admin'), (req, res) => {
  const existing = Prediction.getPrediction(req.params.id);
  if (!existing) return res.status(404).json({ error: 'Pronostic introuvable.' });
  if (!canEdit(req.user, existing)) {
    return res.status(403).json({ error: 'Vous ne pouvez supprimer que vos propres pronostics.' });
  }
  Prediction.deletePrediction(req.params.id);
  res.status(204).end();
});
